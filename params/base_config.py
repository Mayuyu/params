...
try:
    from tensorflow.io.gfile import GFile as fopen  # noqa
except ImportError:
...
class Config(params_dict.ParamsDict):
    ...
    * It recursively enforces a whitelist of basic types and container types,
        so it avoids surprises with copy and reuse caused by unanticipated
        types.
    * It converts dict to Config even within sequences,
    ...
    def _isvalidsequence(cls, v):
        ...
        Returns:
            ...
            type in cls.SEQUENCE_TYPES and element type is in
                cls.IMMUTABLE_TYPES or is dict or ParamsDict.
        """
        if not isinstance(v, cls.SEQUENCE_TYPES):
            return False
        return (
            all(isinstance(e, cls.IMMUTABLE_TYPES) for e in v)
    ...
    def _import_config(cls, v, subconfig_type):
        ...
        if not issubclass(subconfig_type, params_dict.ParamsDict):
            raise TypeError(
                "Subconfig_type should be subclass of ParamsDict, found {!r}".format(  # noqa
                    subconfig_type
                )
            )
        if isinstance(v, cls.IMMUTABLE_TYPES):
            ...
            if not cls._isvalidsequence(v):
                raise TypeError(
                    "Invalid sequence: only supports single level {!r} of {!r} or "  # noqa
            ...
            # Deepcopy here is a temporary solution for preserving type in
    ...
    def _get_subconfig_type(cls, k) -> Type[params_dict.ParamsDict]:
        ...
        Returns:
            ...
            1) returns the type of the annotation if it is subtype of
                ParamsDict;
            2) returns the element type if the annotation of `k` is\
                List[SubType] or Tuple[SubType].
        """
        subconfig_type = Config
        if k in cls.__annotations__:
            # Directly Config subtype.
            type_annotation = cls.__annotations__[k]
            if isinstance(type_annotation, type) and issubclass(
                ...
                if isinstance(field_type, type) and issubclass(
                    field_type, cls.SEQUENCE_TYPES
                ):
                    element_type = getattr(
                        type_annotation, "__args__", [type(None)]
                    )[0]
                    subconfig_type = (
                        element_type
                        if issubclass(element_type, params_dict.ParamsDict)
                        else subconfig_type
                    )
    ...
    def _set(self, k, v):
        ...
        if isinstance(v, dict):
            if k not in self.__dict__ or not self.__dict__[k]:
                # If the key not exist or the value is None, a new
                # Config-family object sould be created for the key.
    ...
    def _override(self, override_dict, is_strict=True):
        ...
        Raises:
            KeyError: overriding reserved keys or keys not exist
                (is_strict=True).
        """
        for k, v in sorted(override_dict.items()):
            ...
            if k not in self.__dict__:
                ...
                if isinstance(v, dict) and self.__dict__[k]:
                    ...
                    isinstance(v, params_dict.ParamsDict) and self.__dict__[k]
                ):
                    self.__dict__[k]._override(
                        v.as_dict(), is_strict
                    )  # pylint: disable=protected-access
    ...
    def replace(self, **kwargs):
        """Like `override`, but returns a copy with the current config
        unchanged.
        """
...
class RuntimeConfig(Config):
    ...
    Attributes:
        ...
        gpu_thread_mode: Whether and how the GPU device uses its own
            threadpool.
...
class TensorboardConfig(Config):
  ...
  Attributes:
    track_lr: Whether or not to track the learning rate in Tensorboard.
        Defaults to True.
