...
class BaseConfigTest(parameterized.TestCase, tf.test.TestCase):
    ...
    """
        ...
        if isinstance(d, base_config.Config.IMMUTABLE_TYPES):
            ...
            for k, v in sorted(d.items()):
                self.assertHasSameTypes(
                    getattr(c, k), v, msg="{}[{!r}]".format(msg, k)
                )
        else:
    ...
    def test_override_is_strict_true(self):
        ...
        with self.assertRaisesRegex(
            AttributeError, "The key `a` does not exist"
        ):
            _ = config.key[0].a
    ...
    def test_config_vs_params_dict(self, v):
        d = {"key": v}
        self.assertEqual(
            type(base_config.Config(d).key[0]), base_config.Config
        )
        self.assertEqual(
    ...
    def test_ppformat(self):
        self.assertEqual(
            ...
            "['s', 1, 1.0, True, None, {}, [], (), {8: 9, (2,): (3, [4], {6: 7})}]",  # noqa
        )
